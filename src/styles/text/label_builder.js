...
LabelBuilder.labelsFromGeometry = function (geometry, { text, size }, label_style) {
    ...
    if (geometry.type === "LineString") {
        ...
        if (width && size.text_size[0] > width) {
            ...
            let label = LabelPoint.explode(text, geometry.coordinates, size, width, line_height, { move_in_tile: false, keep_in_tile: true });

            labels.push(label);
        } else {
            labels.push(new LabelPoint(text, geometry.coordinates, size, null, { move_in_tile: false, keep_in_tile: true }));
        }
        ...
        for (let i = 0; i < points.length; ++i) {
            let point = points[i];
            labels.push(new LabelPoint(text, point, size, null, { move_in_tile: false, keep_in_tile: true }));
        }
    } else if (geometry.type === "Polygon") {
        let centroid = Utils.centroid(geometry.coordinates[0]);
        let area = Utils.polygonArea(geometry.coordinates[0]);

        labels.push(new LabelPoint(text, centroid, size, area, { move_in_tile: false, keep_in_tile: true }));
