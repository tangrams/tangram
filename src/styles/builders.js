...
Builders.buildQuadsForPoints = function (points, vertex_data, vertex_template, vertex_layout_index,
    ...
    let texcoords;
    if (vertex_layout_index.a_texcoord) {
        texcoord_normalize = texcoord_normalize || 1;
    ...
    for (let p=0; p < num_points; p++) {
        ...
        for (let pos=0; pos < 6; pos++) {
            // Add texcoords
            if (vertex_layout_index.a_texcoord) {
                vertex_template[vertex_layout_index.a_texcoord + 0] = texcoords[pos][0] * texcoord_normalize;
                vertex_template[vertex_layout_index.a_texcoord + 1] = texcoords[pos][1] * texcoord_normalize;
            }

            vertex_template[vertex_layout_index.a_position + 0] = point[0];
            vertex_template[vertex_layout_index.a_position + 1] = point[1];

            vertex_template[vertex_layout_index.a_shape + 0] = scaling[pos][0];
            vertex_template[vertex_layout_index.a_shape + 1] = scaling[pos][1];
            vertex_template[vertex_layout_index.a_shape + 2] = angle;
            vertex_template[vertex_layout_index.a_shape + 3] = quad_scale;

            vertex_template[vertex_layout_index.a_offset + 0] = offset[0];
            vertex_template[vertex_layout_index.a_offset + 1] = offset[1];
